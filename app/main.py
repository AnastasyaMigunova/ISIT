...
from ISIT.app.context import Context
...
async def download_image(url, directory, semaphore, session):
...
async def process_page(url, number, directory, session):
    ...
    for img_url in image_urls[:number]:
        tasks.append(download_image(img_url, directory, semaphore, session))
...
async def crawler(next_url, directory, visited_urls, count, number, start_url, session):
    currentNumber = number

    if next_url not in visited_urls:
        try:
            currentNumber = await process_page(next_url, number, directory, session)
    ...
    if currentNumber > 0:
        ...
        if nextPage:
            ...
            await crawler(next_url, directory, visited_urls, count, currentNumber, start_url, session)
        else:
...
async def main(next_url, directory, number: int, visited_urls, start_url, session):

    if not os.path.exists(directory):
        os.makedirs(directory)

    await crawler(next_url, directory, visited_urls, 1, number, start_url, session)
